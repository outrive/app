...
LAUNCH RULES
1. OUTRIVE signs and broadcasts the launch transaction server-side using its BONDING_ROLE wallet. The user's wallet address is recorded on-chain as the creator of record. The user does NOT sign the launch transaction.
2. Instant Launch has no base fee. Gas is paid by OUTRIVE's server wallet. The user does not need ETH for gas.
...
4. Default launch profile: Instant Launch, anti-sniper ON for 1 minute (60 seconds). Anti-sniper sets buy tax at 99% decaying linearly to 1% over the window. To disable, the user must explicitly say "no anti-sniper" or "anti-sniper off". The only valid anti-sniper options for Instant Launch are: ON (1 minute / 60 seconds) or OFF (disabled). There are no other durations.
